...
public import TauCeti.LinearAlgebra.CliffordAlgebra.QuaternionPeriodicity
...
noncomputable def realCliffordEightPeriodicityEquiv (p q : ℕ) :
  ...
  let chain : C (p + 8) q ≃ₐ[ℝ]
    ...
    (realCliffordSignatureSwitchRecurrenceEquiv (p + 6) q).trans <|
      (Algebra.TensorProduct.congr (realCliffordQuaternionRecurrenceEquiv q (p + 4))
        (AlgEquiv.refl : M2 ≃ₐ[ℝ] M2)).trans <|
      (Algebra.TensorProduct.assoc ℝ ℝ ℝ (C (p + 4) q) ℍ[ℝ] M2).trans <|
      (Algebra.TensorProduct.congr
        (realCliffordSignatureSwitchRecurrenceEquiv (p + 2) q)
        (AlgEquiv.refl : ℍ[ℝ] ⊗[ℝ] M2 ≃ₐ[ℝ] ℍ[ℝ] ⊗[ℝ] M2)).trans <|
